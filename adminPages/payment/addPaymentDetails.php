...
<div class="main-content-form">
    <div class="container mt-5">
        <h2 id="formTitle" class="text-center">Add Payment</h2>
        <form id="paymentForm" class="mt-4 shadow p-4 rounded">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="memberName" name="memberName" class="form-control" placeholder="Enter member name" disabled>
            </div>
            ...
            <button type="submit" class="btn btn-sub w-100">Submit Payment</button>
        </form>
...
<script>
    async function fetchMemberData(inputType) {
        ...
        // If member ID is cleared, reset the member name
        ...
        // If member name is cleared, reset the member ID
        ...
        fetch("http://localhost/cs/backend/userManagement/getMemberIDName.php", {
                method: "POST",
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    if (inputType === "id") {
                        document.getElementById("memberName").classList.remove("text-danger");
                        document.getElementById("memNIC").classList.remove("text-danger");
                        document.getElementById("memberName").value = data.memberData.memName || "";
                        document.getElementById("memNIC").value = data.memberData.memNIC || "";
                    } else {
                    ...
                    if (inputType === "id") {
                        document.getElementById("memberName").value = "Not Found";
                        document.getElementById("memberName").classList.add("text-danger");
                        document.getElementById("memNIC").value = "Not Found";
                        document.getElementById("memNIC").classList.add("text-danger");
                    } else {
    ...
    document.getElementById("paymentForm").addEventListener("submit", async function(event) {
        ...
        if (memberName === "" || memberName === "Not Found" ||
            memberID === "" || memberID === "Not Found" ||
            memNIC === "" || memNIC === "Not Found") {
            alert("Invalid Member ID or NIC. Cannot submit payment.");
            return;
        }

        if (!memberID || !memberName || !paymentType || !paymentDate || !dueDate || !amount) {
        ...
        const url = "../backend/paymentManagement/addPayment.php";
        const method = "POST";
